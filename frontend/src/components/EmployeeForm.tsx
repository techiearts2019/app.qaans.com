...
export type EmployeeFormPayload = {
  ...
  photo: { photo?: string; photo_b64?: string };
  projectId?: string;
};
...
type Props = {
  ...
  initialForm?: EmployeeFormState;
  /** In edit mode, the initial photo is a stored value (data URL or http URL)
   *  and should NOT be re-uploaded unless the user re-captures. */
  photoIsPristine?: boolean;
...
export default function EmployeeForm({
  ...
  const [form, setForm] = useState<EmployeeFormState>(
    initialForm ?? emptyEmployeeForm
  );
  const [photoDirty, setPhotoDirty] = useState(!photoIsPristine);
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      // In edit mode, if the user hasn't re-captured the photo we can skip
      // re-encoding entirely — the backend already has the stored URL.
      ...
      if (photoDirty) {
        ...
        if (!photoParts.photo && !photoParts.photo_b64) {
          setToast({
            visible: true,
            message: "Photo did not encode correctly. Please retake the photo.",
            type: "error",
          });
          setSaving(false);
          return;
      ...
      setToast({
        visible: true,
        message: successMessage(form.name),
        type: "success",
      });
      setTimeout(() => router.replace("/employees"), 900);
    } catch (e) {
      console.warn("employee form submit failed", e);
